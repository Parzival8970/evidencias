...
#### 1) Activity Objectives    
_For this exercise, a system will be implemented that allows organizing BLE services and characteristics using a structured table._  

1) Define structures for characteristics and services  
2) Organize data in a table format  
3) Replace conditional logic with table-based logic  
4) Improve scalability of the system  
...
* _**Step 1:** Define structures to represent characteristics and services._  
* _**Step 2:** Create a table containing all services and characteristics._  
* _**Step 3:** Modify event handlers to use the table._  
* _**Step 4:** Replace conditional checks with table iteration._  
* _**Step 5:** Compile, flash, and verify correct operation._  

---

#### 4) Analysis

_The system replaces fixed logic with a data-driven approach._  
_The use of structured tables allows better organization of services and characteristics._  
_This approach improves scalability and simplifies future extensions._  
_The behavior of the system remains the same while the internal structure becomes more flexible._  
...
#define DEVICE_NAME "ESP32C6_BLE_ASUS"
#define ESP_APP_ID  0x55
...
/* Valores */
static const char firmware_version[] = "1.0.0";
static char device_name[32] = "ESP32C6_BLE_ASUS";
static char temperature[8] = "25.0";
...
typedef struct {
    ...
    char_profile_t *chars;
    int char_count;
    uint16_t handle;
} service_profile_t;

/* Características */
static char_profile_t svcA_chars[] = {
    {CHAR_A1_UUID, ESP_GATT_PERM_READ,
     ESP_GATT_CHAR_PROP_BIT_READ,
     (uint8_t*)firmware_version, 5, 5, true, 0},

    {CHAR_A2_UUID, ESP_GATT_PERM_READ|ESP_GATT_PERM_WRITE,
     ESP_GATT_CHAR_PROP_BIT_READ|ESP_GATT_CHAR_PROP_BIT_WRITE,
     (uint8_t*)device_name, 17, 32, false, 0}
};

static char_profile_t svcB_chars[] = {
    {CHAR_B1_UUID, ESP_GATT_PERM_READ,
     ESP_GATT_CHAR_PROP_BIT_READ,
     (uint8_t*)temperature, 4, 8, true, 0},

    {CHAR_B2_UUID, ESP_GATT_PERM_WRITE,
     ESP_GATT_CHAR_PROP_BIT_WRITE,
     NULL, 0, 64, false, 0}
};

/* Servicios */
static service_profile_t services[] = {
    {SVC_A_UUID, 6, svcA_chars, 2, 0},
    {SVC_B_UUID, 6, svcB_chars, 2, 0}
};

#define SERVICE_COUNT 2

static int current_service = 0;
...
static void create_service(esp_gatt_if_t gif, uint16_t uuid, uint16_t num)
{
    esp_gatt_srvc_id_t id = {
        .is_primary = true,
        .id.inst_id = 0,
    ...
    esp_ble_gatts_create_service(gif, &id, num);
}

static void add_char(uint16_t svc, char_profile_t *c)
{
    esp_bt_uuid_t cuuid = {.len=ESP_UUID_LEN_16, .uuid.uuid16=c->uuid};

    esp_attr_value_t val = {
        .attr_max_len = c->max_len,
        .attr_len = c->val_len,
        .attr_value = c->value
    };

    esp_attr_control_t ctrl = {
        .auto_rsp = c->auto_rsp ? ESP_GATT_AUTO_RSP : ESP_GATT_RSP_BY_APP
    };

    esp_ble_gatts_add_char(svc, &cuuid, c->perm, c->prop, &val, &ctrl);
}
                                        ...
                                        esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_REG_EVT:
        ...
        esp_ble_gap_set_device_name(DEVICE_NAME);
        esp_ble_gap_config_adv_data(&adv_data);
        create_service(gatts_if, services[0].uuid, services[0].num_handles);
        break;
    ...
    case ESP_GATTS_CREATE_EVT: {
        ...
        for (int i=0;i<SERVICE_COUNT;i++){
            if (services[i].uuid == uuid) {
                ...
                services[i].handle = param->create.service_handle;
                esp_ble_gatts_start_service(services[i].handle);

                current_service = i;
                current_char = 0;

                add_char(services[i].handle, &services[i].chars[0]);
            }
        }
        break;
    }
    ...
    case ESP_GATTS_ADD_CHAR_EVT: {
        char_profile_t *c = &services[current_service].chars[current_char];
        c->handle = param->add_char.attr_handle;

        current_char++;

        if (current_char < services[current_service].char_count) {
            add_char(services[current_service].handle,
                     &services[current_service].chars[current_char]);
        } else {
            current_service++;
            if (current_service < SERVICE_COUNT) {
                create_service(gatts_if_global,
                               services[current_service].uuid,
                               services[current_service].num_handles);
            } else {
                ESP_LOGI(TAG, "TABLE SETUP DONE");
            }
        }
        break;
    }
    ...
    case ESP_GATTS_WRITE_EVT: {
        for(int i=0;i<SERVICE_COUNT;i++){
            for(int j=0;j<services[i].char_count;j++){
                if(param->write.handle == services[i].chars[j].handle){
                    ESP_LOGI(TAG,"Write to UUID: 0x%X", services[i].chars[j].uuid);
                }
            }
        }
        break;
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "BLE READY (TABLE MODE)");
}
```
